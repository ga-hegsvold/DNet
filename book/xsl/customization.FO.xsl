...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  version="1.0">
    ...
    <!--
     ...
     | ========================================================================
     +-->
    <xsl:variable name="dnet.fg.color">brown</xsl:variable>
    <xsl:variable name="dnet.bg.color">#EEEECC</xsl:variable>
    ...
    <xsl:variable name="dnet.chapter.title.bg.color"><xsl:value-of select="$dnet.bg.color"/></xsl:variable>
    <xsl:variable name="dnet.chapter.title.border.color"><xsl:value-of select="$dnet.fg.color"/></xsl:variable>
    <xsl:variable name="dnet.chapter.title.border.style">solid</xsl:variable>
    <xsl:variable name="dnet.chapter.title.border.width">thick</xsl:variable>
    ...
    -->
    <!--
     | Put an fo:block with unique ID on last page of document.
     | This fo:block's ID is referenced in footer template to fetch last page.
    -->
    <!--
    <xsl:template match="book/chapter[last()]">
    ...
    <xsl:template name="table.cell.properties"> 
      ...
      <xsl:variable name="bgcolor">
	    ...
	    <xsl:value-of select="$dnet.table.head.bg.color" />
          </xsl:when>
    ...
    <!--
     ...
     | ========================================================================
     +-->
    <xsl:param name="orderedlist.label.width">15pt</xsl:param>
    ...
    <!-- move to customization.FO.header.xsl ? -->
    ...
    <!-- move to customization.FO.footer.xsl ? -->
    ...
    <!--
     ...
     +-->
     <!--
     "always" makes large tables flow out of page area
     "auto" makes some crappy brakes in informalexamples
     -->
    ...
    <xsl:attribute-set name="component.title.properties">
      <xsl:attribute name="background-color"><xsl:value-of select="$dnet.chapter.title.bg.color" /></xsl:attribute>
      <xsl:attribute name="border-color"><xsl:value-of select="$dnet.chapter.title.border.color" /></xsl:attribute>
      <xsl:attribute name="border-top-style"><xsl:value-of select="$dnet.chapter.title.border.style" /></xsl:attribute>
      <xsl:attribute name="border-top-width"><xsl:value-of select="$dnet.chapter.title.border.width" /></xsl:attribute>
